...
namespace CS5410.Objects
{
    ...
    class MyRandom : Random
    {
        ...
        public Vector2 nextPointAtAngle(float landerAngle)
        {
            float angle = landerAngle + (float)(this.NextDouble() * 0.5 - 0.3); 
            // ensure the angle is within the range of landerAngle +- 0.3
            if (angle < landerAngle - 0.3) angle = landerAngle - 0.3f;
            if (angle > landerAngle + 0.3) angle = landerAngle + 0.3f;
