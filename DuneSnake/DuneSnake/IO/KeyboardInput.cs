...
namespace CS5410.IO
{
    ...
    public class KeyboardInput : IInputDevice
    {
        ...
        public Control Thrust = new Control(Keys.Up);
        [DataMember(Name = "Left")]
        public Control Left = new Control(Keys.Left);
        [DataMember(Name = "Right")]
        public Control Right = new Control(Keys.Right);
                    ...
                    if (storage.FileExists("Controls.json")) // check if it exists before trying to open it
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("Controls.json", FileMode.Open))
                        {
                            if (fs != null)
                            {
                                ...
                                this.Thrust = m_loadedState.Thrust;
                                this.Left = m_loadedState.Left;
                                this.Right = m_loadedState.Right;
                            }
                        }
                    }
                }
